...
public class DragSortListView extends ListView {
	...
	public volatile MotionEvent evDown;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		boolean more = false, supperCalled = false;
		...
		if (mDragState == DRAGGING) {
			...
			switch (action) {
				case MotionEvent.ACTION_UP:
					if (evDown != null) {
						float diffX = Math.abs(ev.getX() - evDown.getX());
						float diffY = Math.abs(ev.getY() - evDown.getY());
						if (diffX < 20 && diffY < 20) {
							int pos = pointToPosition((int) ev.getX(),
									(int) ev.getY());
							// if(!supperCalled){
							// doActionUpOrCancel();
							...
							if (mOnItemSelectedListener != null)
								mOnItemSelectedListener.onItemSelected(pos);
							Log.i("Diff", "Diff: diffX:" + diffX + " :: diffY:"
									+ diffY + " ::: Pos:" + pos);
						}
					}
