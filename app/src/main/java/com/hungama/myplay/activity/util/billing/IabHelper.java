...
public class IabHelper {
	...
	public IabHelper(String fragmentTag, Context ctx, String base64PublicKey) {
		...
		// FlurryAgent.onStartSession(ctx,
		// ctx.getString(R.string.flurry_app_key));
		Analytics.startSession(ctx);
	}
	...
	public void startSetup(final OnIabSetupFinishedListener listener)
		...
		try {
			...
			Logger.e(getClass().getName() + ":312", e.toString());
		}
	}
	...
	public void launchPurchaseFlow(Activity act, String sku, int requestCode,
			OnIabPurchaseFinishedListener listener) throws Exception {
...
/**
    ...
    */
	...
	public Inventory queryInventory(boolean querySkuDetails,
		...
		try {
			...
			if (r != BILLING_RESPONSE_RESULT_OK
					&& r != BILLING_RESPONSE_RESULT_ITEM_ALREADY_OWNED) {
	 ...
	 */
	public void queryInventoryAsync(final Handler handler,
			final boolean querySkuDetails, final List<String> moreSkus,
			final QueryInventoryFinishedListener listener) throws Exception {
		checkNotDisposed();
		checkSetupDone("queryInventory");
		...
		(new Thread(new Runnable() {
			public void run() {
				try {
					...
					IabResult result = new IabResult(
					...
					try {
						...
						logDebug("queryInventoryAsync 3");
						ex.printStackTrace();
						result = ex.getResult();
					}

					logDebug("queryInventoryAsync 4");
					...
					if (!mDisposed && listener != null) {
						handler.post(new Runnable() {
							public void run() {
								listener.onQueryInventoryFinished(result_f,
										inv_f);
							}
						});

					} else
						logDebug("queryInventoryAsync 6");
				} catch (Exception e) {
					e.printStackTrace();
					flagEndAsync();
					logDebug("queryInventoryAsync 7");
				}
			}
		})).start();
	}
