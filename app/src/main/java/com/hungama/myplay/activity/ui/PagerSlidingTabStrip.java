...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs,
		...
		// tabTextSize = (int)
		// TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, tabTextSize,
		// dm);
		...
		// tabTextSize=
		// (int)getResources().getDimension(R.dimen.xlarge_text_size);
	...
	private void addTextTab(final int position, String title) {
		...
		View itemLayoutView = LayoutInflater.from(getContext()).inflate(
				R.layout.list_item_pagerslidingtab, null);

		LanguageTextView tab = (LanguageTextView) itemLayoutView;// new
																	// LanguageTextView(getContext());
		tab.setText(Utils.getMultilanguageTextLayOut(getContext(), title));
	...
	private void updateTabStyles() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof LanguageTextView) {
				...
				LanguageTextView tab = (LanguageTextView) v;
...
//				tab.setTypeface(tabTypeface, tabTypefaceStyle);
				...
				if (textAllCaps) {
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
						...
						tab.setText(Utils.getMultilanguageTextLayOut(
								getContext(), tab.getText().toString()
										.toUpperCase(locale)));
