...
public class GifDecoder {
	...
	public Bitmap getNextFrame() {
		...
		if (act == null) {
			Logger.w(TAG, "No Valid Color Table");
	...
	// if (is != null) {
	// try {
	// final int capacity = (contentLength > 0) ? (contentLength + 4096)
	// : 4096;
		...
		if (previousFrame != null
			...
			if (previousFrame.dispose == DISPOSAL_BACKGROUND) {
				...
				for (int i = 0; i < previousFrame.ih; i++) {
					final int n1 = (previousFrame.iy + i) * width
							+ previousFrame.ix;
	...
	protected int readBlock() {
		...
		if (blockSize > 0) {
			try {
				...
				Logger.w(TAG, "Error Reading Block");
	...
	protected int[] readColorTable(final int ncolors) {
		...
		try {
			...
			Logger.w(TAG, "Format Error Reading Color Table");
			Logger.printStackTrace(e);
			status = STATUS_FORMAT_ERROR;
		}
		return tab;
	...
	protected void readGraphicControlExt() {
		...
		if (currentFrame.dispose == 0) {
			currentFrame.dispose = 1; // elect to keep old image if
										// discretionary
		}
	...
	protected void readBitmap() {
		...
		currentFrame.bufferFrameStart = rawData.position(); // Save this as the
															// decoding position
															// pointer
		skipBitmapData();
		if (err()) {
	...
	protected void readLSD() {
		...
		try {
			...
			previousImage = Bitmap.createBitmap(width / 2, height / 2,
					Bitmap.Config.RGB_565);
			currentImage = Bitmap.createBitmap(width / 2, height / 2,
					Bitmap.Config.RGB_565);
		}
	}
