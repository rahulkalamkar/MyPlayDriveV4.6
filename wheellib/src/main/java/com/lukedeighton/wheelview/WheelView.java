...
public class WheelView extends View {
	...
	// The touch factors decrease the drag movement towards the center of the
	// wheel. It is there so
	// that dragging the wheel near the center doesn't cause the wheel's angle
	// to change
	// drastically. It is squared to provide a linear function once multiplied
	// by 1/r^2
	...
	static {
		...
		for (int i = 0; i < size; i++) {
			...
			TOUCH_FACTORS[i] = (1 - factor * factor / numerator)
					* TOUCH_DRAG_COEFFICIENT;
		}
	}
	...
	/**
	 * The item angle is the angle covered per item on the wheel and is in
	 * degrees. The {@link #mItemAnglePadding} is included in the item angle.
	 */
	private float mItemAngle;

	/**
	 * Angle padding is in degrees and reduces the wheel's items size during
	 * layout
	 */
	private float mItemAnglePadding;

	/**
	 * Selection Angle is the angle at which an item is considered selected. The
	 * {@link #mOnItemSelectListener} is called when the 'most selected' item
	 * changes.
	 */
	...
	/**
	 * Wheel item bounds are always pre-rotation and based on the
	 * {@link #mSelectionAngle}
	 */
...
//	private WheelSelectionTransformer mSelectionTransformer;
	...
	public WheelView(Context context, AttributeSet attrs, int defStyle) {
		 ...
		 * attr = a.getIndex(i); switch (attr) { case
		 * com.android.internal.R.styleable.View_background: background =
		 * a.getDrawable(attr); break;
		 */

		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.WheelView, defStyle, 0);
		...
		if (d != null) {
			...
			int color = a.getColor(R.styleable.WheelView_emptyItemColor,
					NEVER_USED);
		...
		if (d != null) {
			...
			int color = a
					.getColor(R.styleable.WheelView_wheelColor, NEVER_USED);
			setWheelColor(color);
		}
		...
		if (d != null) {
			...
			int color = a.getColor(R.styleable.WheelView_selectionColor,
					NEVER_USED);
			setSelectionColor(color);
		}

		mSelectionPadding = a.getDimensionPixelSize(
				R.styleable.WheelView_selectionPadding, 0);
		
//		mSelectionPadding = a.getDimensionPixelSize(
//				R.styleable.WheelView_wheelToItemDistance, 0);
//		
		
		
		
		mIsRepeatable = a.getBoolean(R.styleable.WheelView_repeatItems, false);
		mIsWheelDrawableRotatable = a.getBoolean(
				R.styleable.WheelView_rotatableWheelDrawable, true);
		mSelectionAngle = a.getFloat(R.styleable.WheelView_selectionAngle, 0f);
		setWheelRadius(a.getLayoutDimension(R.styleable.WheelView_wheelRadius,
				0 /* TODO Wrap_content */));
		mOffsetX = a.getDimensionPixelSize(R.styleable.WheelView_wheelOffsetX,
				0);
		mOffsetY = a.getDimensionPixelSize(R.styleable.WheelView_wheelOffsetY,
				0);
		...
		mItemAnglePadding = a.getFloat(
				R.styleable.WheelView_wheelItemAnglePadding, 0f); // TODO angle
																	// works
																	// with the
																	// ItemRadius
		...
		if (itemCount != 0) {
			...
			float itemAngle = a.getFloat(R.styleable.WheelView_wheelItemAngle,
					0f);
			if (itemAngle != 0f) {
		...
		mItemRadius = a.getDimensionPixelSize(
				R.styleable.WheelView_wheelItemRadius, 0);

		if (mItemCount == 0 && mWheelToItemDistance > 0) {
			mItemAngle = calculateAngle(mRadius, mWheelToItemDistance)
					+ mItemAnglePadding;
			setWheelItemAngle(mItemAngle);
		}

		String itemTransformerStr = a
				.getString(R.styleable.WheelView_wheelItemTransformer);
		if (itemTransformerStr != null) {
			mItemTransformer = validateAndInstantiate(itemTransformerStr,
					WheelItemTransformer.class);
		}

//		String selectionTransformerStr = a
//				.getString(R.styleable.WheelView_selectionTransformer);
//		if (selectionTransformerStr != null) {
//			mSelectionTransformer = validateAndInstantiate(
//					selectionTransformerStr, WheelSelectionTransformer.class);
//		}

		mWheelPadding = a.getDimensionPixelSize(
				R.styleable.WheelView_wheelPadding, 0);

		mWheelPosition = a.getInt(R.styleable.WheelView_wheelPosition, 0);
		if (!a.hasValue(R.styleable.WheelView_selectionAngle)) {
			// TODO use gravity to default the selection angle if not already
			// specified
		}
	...
	private <T> T validateAndInstantiate(String clazzName,
		...
		try {
			...
			if (clazz.isAssignableFrom(xmlClazz)) {
				try {
					...
					errorMessage = "No argumentless constructor for "
				...
				errorMessage = "Class inflated from xml ("
						+ xmlClazz.getSimpleName() + ") does not implement "
						+ clazz.getSimpleName();
				instance = null;
			}
		} catch (ClassNotFoundException e) {
			errorMessage = clazzName
					+ " class was not found when inflating from xml";
			instance = null;
		}
	...
	public void initWheelView() {
		// TODO I only really need to init with default values if there are non
...
//		mSelectionTransformer = new FadingSelectionTransformer();
	...
	public static interface OnWheelItemVisibilityChangeListener {
		void onItemVisibilityChange(WheelAdapter adapter, int position,
				boolean isVisible);
	}

	/* TODO public */void setOnWheelItemVisibilityChangeListener(
			OnWheelItemVisibilityChangeListener listener) {
	...
	public void setOnWheelAngleChangeListener(
			OnWheelAngleChangeListener listener) {
	...
	public void setWheelItemTransformer(WheelItemTransformer itemTransformer) {
		if (itemTransformer == null)
			throw new IllegalArgumentException(
					"WheelItemTransformer cannot be null");
		mItemTransformer = itemTransformer;
	}

//	public void setWheelSelectionTransformer(
//			WheelSelectionTransformer transformer) {
...
//	}
	...
	/**
	 * <p>
	 * When true the wheel drawable is rotated as well as the wheel items. For
	 * performance it is better to not rotate the wheel drawable.
	 * <p>
	...
	/*
	 * TODO public void setWheelItemRadius(float radius, int itemCount) {
	 * mItemRadius = radius; mItemAngle = calculateItemAngle(itemCount);
	...
	/**
	 * Find the largest circle to fit within the item angle. The point of
	 * intersection occurs at a tangent to the wheel item.
	 */
	private float calculateWheelItemRadius(float angle) {
		return (float) (mWheelToItemDistance * Math.sin(Math
				.toRadians((double) ((angle - mItemAnglePadding) / 2f))));
	}

	private float calculateAngle(float innerRadius, float outerRadius) {
		return 2f * (float) Math.toDegrees(Math
				.asin((double) (innerRadius / outerRadius)));
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (mWidth != width || mHeight != height || mLeft != left
				|| mTop != top) {
	...
	private void layoutWheelItems() {
		...
		for (int i = 0; i < mItemCount; i++) {
			...
			float x = mWheelBounds.mCenterX + mWheelToItemDistance
					* (float) Math.cos(angle);
			float y = mWheelBounds.mCenterY + mWheelToItemDistance
	...
	/**
	 * You should set the wheel drawable not to rotate for a performance
	 * benefit. See the method {@link #setWheelDrawableRotatable(boolean)}
	 */
	...
	/**
	 * If the drawable has infinite lines of symmetry then you should set the
	 * wheel drawable to not rotate, see
	 * {@link #setWheelDrawableRotatable(boolean)}. In other words, if the
	 * drawable doesn't look any different whilst it is rotating, you should
	 * improve the performance by disabling the drawable from rotating.
	 */
	...
	public void setPosition(int position) {
		setAngle(getAngleForPosition(position), true);
	}
	...
	int xxSelectedPos=-321;

	public void setAngle(float angle, boolean isLAst) {

		Log.e("setAngle", angle+":"+isLAst);
		mAngle = angle;
		updateSelectionPosition(isLAst);
	...
	private void updateSelectionPosition(boolean islastCall) {
		int selectedPosition = (int) ((-mAngle + -0.5 * Math.signum(mAngle)
				* mItemAngle) / mItemAngle);
		if (islastCall)
			xxSelectedPos = selectedPosition;
	...
	private void setSelectedPosition(int position) {
		...
		if (mOnItemSelectListener != null) {
			mOnItemSelectListener.onWheelItemSelected(this,
					getSelectedPosition());
		}
	}
	...
	public boolean onTouchEvent(@NonNull MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			float touchFactor = TOUCH_FACTORS[(int) (touchRadiusSquared
					/ mRadiusSquared * TOUCH_FACTORS.length)];
			float touchAngle = mWheelBounds.angleToDegrees(x, y);
			float draggedAngle = -1f
					* Circle.shortestAngle(touchAngle, mLastTouchAngle)
					* touchFactor;
	...
	private void flingWheel() {
		...
		// torque = r X F
		mForceVector.set(mVelocityTracker.getXVelocity(),
		...
		// estimate an angular velocity based on the strength of the angular
		// acceleration
	...
	public int rawPositionToAdapterPosition(int position) {
		return mIsRepeatable ? Circle.clamp(position, mAdapterItemCount)
				: position;
	}

	public int rawPositionToWheelPosition(int position) {
		return rawPositionToWheelPosition(position,
				rawPositionToAdapterPosition(position));
	}

	public int rawPositionToWheelPosition(int position, int adapterPosition) {
		int circularOffset = mIsRepeatable ? ((int) Math
				.floor((position / (float) mAdapterItemCount)) * (mAdapterItemCount - mItemCount))
				: 0;
	...
	private void update(float deltaTime) {
		...
		if (vel > 0f) {
			mAngularVelocity -= velSqr * VELOCITY_FRICTION_COEFFICIENT
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mWheelDrawable != null) {
			if (mIsWheelDrawableRotatable) {
				canvas.save();
				canvas.rotate(angle, mWheelBounds.mCenterX,
						mWheelBounds.mCenterY);
		...
		Log.e("Selected", "x>>" + ">>>"+xxSelectedPos);
		for (int i = offset; i < length; i++) {

			if (xxSelectedPos>=0 && mSelectedPosition == i) 
			{
//				xxSelectedPos=-321;
				continue;
			}
			...
			int wheelItemPosition = rawPositionToWheelPosition(i,
					adapterPosition);
//			Log.e("Drawing", ">>" + i + " Adapter" + adapterPosition
//					+ "  wheelItemPosition" + wheelItemPosition);
			...
			if (Rect.intersects(sTempRect, mViewBounds)) {
				...
				if (!cacheItem.mIsVisible) {
					...
					if (mOnItemVisibilityChangeListener != null) {
						mOnItemVisibilityChangeListener.onItemVisibilityChange(
								mAdapter, adapterPosition, true);
					}
				}

				if (i == mSelectedPosition && mSelectionDrawable != null) {
//					mSelectionDrawable.setBounds(sTempRect.left
//							- mSelectionPadding, sTempRect.top
//							- mSelectionPadding, sTempRect.right
//							+ mSelectionPadding, sTempRect.bottom
//							+ mSelectionPadding);
//					mSelectionTransformer.transform(mSelectionDrawable,
//							itemState);
//					mSelectionDrawable.draw(canvas);
				...
				if (cacheItem.mIsVisible) {
					...
					if (mOnItemVisibilityChangeListener != null) {
						mOnItemVisibilityChangeListener.onItemVisibilityChange(
								mAdapter, adapterPosition, false);
					}
				}
			}
		}
	...
	private void updateItemState(ItemState itemState, int adapterPosition,
		...
		float angleFromSelection = Circle.shortestAngle(itemAngle,
				mSelectionAngle);
	...
	/**
	 * The ItemState is used to provide extra information when transforming the
	 * selection drawable or item bounds. It is also used to
	 */
	...
	/**
	 * A simple class to represent a vector with an add and cross product
	 * method. Used only to calculate the Wheel's angular velocity in
	 * {@link #flingWheel()}
	 */
	static class Vector {
		float x, y;
