...
import io.techery.progresshint.addition.R;
...
public abstract class ProgressHintDelegate implements OnSeekBarChangeListener {
  ...
  protected PopupWindow mPopup, mPopupText;
  protected View mPopupView;
  protected ImageView mPopupPointerView;
  ...
  private void initHintPopup() {
    ...
    mPopupText = new PopupWindow(mPopupView, WRAP_CONTENT, WRAP_CONTENT, false);
    mPopupText.setAnimationStyle(mPopupAnimStyle);

    mPopupPointerView = new ImageView(mSeekBar.getContext());
    mPopupPointerView.setImageResource(R.drawable.popup_pointer);
    mPopupPointerView.measure(makeMeasureSpec(0, UNSPECIFIED), makeMeasureSpec(0, UNSPECIFIED));
    mPopup = new PopupWindow(mPopupPointerView, WRAP_CONTENT, WRAP_CONTENT, false);
  ...
  private void showPopupInternally() {
    ...
    int posx = offsetPoint.x;
    if(posx > mPopupTextView.getMeasuredHeight()/2){
      posx = offsetPoint.x - mPopupTextView.getMeasuredHeight()/2;
    } else{
      posx = offsetPoint.x - 10;
    }
    mPopupText.showAtLocation(mSeekBar, Gravity.NO_GRAVITY, 0, 0);
    mPopupText.update(mSeekBar, posx, offsetPoint.y - mPopupTextView.getMeasuredHeight(), -1,
            -1);
  }
  ...
  public void hidePopup() {
    ...
    if (mPopupText.isShowing()) mPopupText.dismiss();
  }
  ...
  public void setPopupLayout(@LayoutRes int layout) {
    ...
    if (mPopupText != null) mPopupText.dismiss();
    initHintPopup();
    checkInitialState();
  }
...
//    if(progress<5){
//      mPopupView.setBackgroundResource(R.drawable.progress_bg_left);
//    } else {
//      mPopupView.setBackgroundResource(R.drawable.progressbg);
//    }
    String popupText = null;
    if (mHintAdapter != null) {
...
//      System.out.println("Position :::: x " + offsetPoint.x + "  y : " + offsetPoint.y);
      mPopup.update(mSeekBar, offsetPoint.x, offsetPoint.y, -1, -1);
      ...
      mPopupText.update(mSeekBar, posx, offsetPoint.y - mPopupTextView.getMeasuredHeight(), -1, -1);
    }
  }
...
//    System.out.println("Position :::: " + ((int) (progress * (mSeekBar.getWidth()
